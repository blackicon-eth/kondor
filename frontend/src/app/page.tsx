  ...
  return (
    <main className="flex justify-center items-center w-full h-full">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-linear-to-br from-surface via-surface-container-low to-surface" />
      ...
      <div className="container px-8 z-20 h-full">
        <div className="grid lg:grid-cols-2 gap-32 items-center h-full">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              >
                The privacy first automation engine for your tokens. Route, swap, and stake assets
                the moment they arrive, all anonymously.
              </motion.p>
            </div>

            <motion.div custom={3} variants={fadeUp}>
              <Button variant="primary" size="hero" onClick={handleLaunchApp}>
                Launch App
              </Button>
            </motion.div>
            ...
            >
              {tech.map((item) => (
                <div
                  key={item.name}
                  className="flex flex-col justify-center items-start space-y-1 w-full shrink-0"
                >
                  <div
                    className={`text-2xl font-black font-headline uppercase tracking-wide whitespace-nowrap ${item.highlight ? "text-primary-container" : "text-on-surface"}`}
                  >
                    {item.name}
                  </div>
                  <div className="text-sm text-secondary-ds font-label">{item.description}</div>
                </div>
              ))}
            </motion.div>
          </motion.div>
          ...
          >
            <div className="relative w-full max-w-136 aspect-square">
              {/* Logo container */}
              <div
                className="absolute inset-0 bg-surface-container-lowest crimson-glow"
                style={{
