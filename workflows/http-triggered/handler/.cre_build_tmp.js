...
var init_sha2 = __esm(() => {
  ...
  SHA512 = class SHA5122 extends HashMD {
...
function edwards(params, extraOpts = {}) {
  ...
  class Point {
    ...
    static fromBytes(bytes, zip215 = false) {
      const len2 = Fp.BYTES;
      const { a, d } = CURVE;
      bytes = copyBytes(_abytes2(bytes, len2, "point"));
...
function eddsa(Point, cHash, eddsaOpts = {}) {
  ...
  function verify(sig, msg, publicKey, options = verifyOpts) {
    ...
    sig = ensureBytes("signature", sig, len2);
    msg = ensureBytes("message", msg);
    ...
    const s = bytesToNumberLE(sig.subarray(mid, len2));
    let A, R, SB;
    try {
...
class _RistrettoPoint extends PrimeEdwardsPoint {
  ...
  static fromBytes(bytes) {
    ...
    const y = mod2(u1 * Dy);
    const t = mod2(x * y);
    if (!isValid2 || isNegativeLE(t, P) || y === _0n7)
...
function ed25519PrivToX25519(priv) {
  return ed25519.utils.toMontgomeryPriv(priv);
}
function b64Decode(s) {
  let base64 = s.replace(/-/g, "+").replace(/_/g, "/");
  const pad2 = base64.length % 4;
  if (pad2)
    base64 += "=".repeat(4 - pad2);
...
function fromStorageFormat(value2) {
  return value2.startsWith(ENCRYPTED_PREFIX) ? value2 : ENCRYPTED_PREFIX + value2;
}
function computeSharedSecret(privateKey, publicKey) {
  return x25519.getSharedSecret(privateKey, publicKey);
}
function deriveAesKey(sharedSecret) {
  return hkdf(sha2562, sharedSecret, undefined, undefined, 16);
}
function decrypt(encryptedPayload, privateKey, otherPartyPublicKey) {
  const payload = fromStorageFormat(encryptedPayload);
  const raw = payload.startsWith(ENCRYPTED_PREFIX) ? payload.slice(ENCRYPTED_PREFIX.length) : payload;
  ...
  if (!pubKey)
    throw new Error("Missing other party public key");
...
function isEncrypted(value2) {
  if (typeof value2 !== "string")
    return false;
  if (value2.startsWith(ENCRYPTED_PREFIX))
    return true;
...
function evaluateBranch(branch, prices, log) {
...
function resolveSepoliaAddress(symbol2) {
  const addr = SEPOLIA_TOKENS[symbol2.toUpperCase()] ?? SEPOLIA_TOKENS[symbol2];
  if (!addr) {
    throw new Error(`No Sepolia address for token: ${symbol2}`);
  }
  return addr;
}
var onHttpTrigger = (runtime2, payload) => {
  ...
  const quotes = selectedActions.filter((action) => action.percent > 0 && action.actionType === "swap").map((action) => {
    const inputAmountAtomic = BigInt(Math.floor(intent.inputAmount * action.percent / 100 * 10 ** intent.inputDecimals)).toString();
    return {
  ...
  runtime2.log(`Built ${quotes.length} swap quote(s) for Sepolia`);
  ...
  try {
    ...
    if (parsed.error) {
      ...
      runtime2.log(`SIMULATION OK: ${parsed.result}`);
    }
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    runtime2.log(`SIMULATION ERROR: ${msg}`);
  }
  ...
  runtime2.log(`SUCCESS: report submitted for salt=${salt}, ${targets.length} batch calls encoded`);
